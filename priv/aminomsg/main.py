...
def chid(stdscr, subclient):
    ...
    while True:
        ...
        if not cid.startswith("http://aminoapps.com/p/"):
            ...
            stdscr.getch()
            continue
        cid = client.get_from_code(cid).objectId
        break
    while True:
        stdscr.move(max.ceny-4, 0);stdscr.clrtobot()
...
def com(stdscr, com):
    ...
    while True:
        ...
        stdscr.addstr(7, 0, f"comId: {subclient.comId}")
        x = menu(stdscr, max.ceny, max.cenx-10, {"Enviar mensaje": lambda: chid(stdscr, subclient), "Volver": lambda: 1})
        if x:
            return
